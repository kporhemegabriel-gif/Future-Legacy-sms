    ...
    <div className="space-y-5">
      <div className="border border-hairline bg-paper p-6 max-w-xl">
        ...
        <dl className="text-sm space-y-3">
          ...
          <Row
            ...
            value={error ? `Unreachable — ${error}` : health ? "Connected" : "Checking…"}
            tone={error ? "rust" : health ? "forest" : "slate"}
          />
          <Row
            label="Server uptime"
            value={health ? `${Math.round(health.uptime)}s` : "—"}
          />
      ...
      <div className="border border-hairline bg-paper p-6 max-w-xl text-sm text-slate leading-relaxed">
        ...
        <p>
          ...
          <code className="font-mono text-ink">server/.env</code>, pointed at your Joytree cluster.
          Connection health is logged in the server console on startup.
        </p>
      </div>
  ...
  return (
    <div className="border border-hairline bg-paper p-6 max-w-xl">
      ...
      <p className="text-xs text-slate mb-4">
        Only admins can create new accounts — public sign-up is disabled after
        the first admin is set up.
      </p>

      {message && (
        <div className="border border-forest/40 bg-forest/5 text-forest text-sm px-3 py-2 mb-4">
          {message}
        </div>
      )}
      {error && (
        <div className="border border-rust/40 bg-rust/5 text-rust text-sm px-3 py-2 mb-4">
          {error}
        </div>
      )}
